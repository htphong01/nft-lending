...
import { getRandomInt, getEncodeOffer, getEncodedSignature, getMessage, getTimestamp, skipTime, ZERO_ADDRESS, MAX_UINT256 } from "./utils";
...
describe("Loan", () => {
    ...
    beforeEach(async () => {
        ...
        const lendingPool = await LendingPool.deploy(deployer.address);

        const lendingStake = await LendingStake.deploy(wXENE.address, lendingPool.address, "10000000000000000000", 0);
