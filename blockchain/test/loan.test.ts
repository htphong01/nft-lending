...
describe("Loan", () => {
  ...
  describe.only("acceptOffer", () => {
    ...
    it.skip("should revert with lender signature is invalid", async () => {
    ...
    describe("should revert with lender signature is invalid", async () => {
      ...
      it("Should revert if loan contract is zero address", async () => {
        signature.signature = await getOfferSignature(offer, signature, lender, ZERO_ADDRESS);
      ...
      it("Should revert if lender is lending pool but signer is not admin", async () => {
        offer.lendingPool = lendingPool.address;
        signature.signature = await getOfferSignature(offer, signature, lender, directLoanFixedOffer.address);
      ...
      it("Should revert if sign wrong data", async () => {
        signature.signature = await getOfferSignature(offer, signature, lender, directLoanFixedOffer.address);
        offer.principalAmount = TOKEN_1.mul(2);
    ...
    it("should revert with transfer amount exceeds balance", async () => {
      await chonkSociety.connect(borrower).approve(directLoanFixedOffer.address, 1);
      await wXENE.connect(lender).transfer(accounts[0].address, TOKEN_1.mul(100));
  ...
  describe("payBackLoan", () => {
    ...
    it("should revert with insufficient allowance", async () => {
      await wXENE.connect(borrower).approve(directLoanFixedOffer.address, 1);
    ...
    it("should revert with transfer amount exceeds balance", async () => {
      await wXENE.connect(borrower).approve(directLoanFixedOffer.address, MAX_UINT256);
      await wXENE.connect(borrower).transfer(accounts[0].address, TOKEN_1.mul(100));
      await expect(directLoanFixedOffer.connect(borrower).payBackLoan(loanId)).to.revertedWith(
        "ERC20: transfer amount exceeds balance"
      );
