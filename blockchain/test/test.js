...
describe.only("Loan", () => {
    ...
    describe("acceptOffer", () => {
        ...
        it.only("should accept offer successfully", async () => {
            offer = {
                principalAmount: TOKEN_1,
                maximumRepaymentAmount: TOKEN_1,
                nftCollateralId: 1,
            ...
            signature = {
                signer: lender.address,
                expiry: (await getTimestamp()) + offer.duration,
                nonce: getRandomInt(),
            };
            ...
            const payload = ethers.utils.solidityPack(["bytes", "bytes", "address", "uint256"], [encodedOffer, encodedSignature, directLoanFixedOffer.address, 31337]);

            const message = ethers.utils.arrayify(ethers.utils.keccak256(payload));

            signature.signature = lender.signMessage(message);
            ...
            // Test Renegotiate Loan
            const newDuration = offer.duration + ONE_DAY;
            const lenderSignature = {
                signer: lender.address,
                nonce: getRandomInt(),
                expiry: (await getTimestamp()) + newDuration,
            };
            const renegotiateFee = TOKEN_1.div(100);

            const encodedLenderSignature = getEncodedSignature(lenderSignature);

            const renegotiationPayload = ethers.utils.solidityPack(
                ["bytes32", "uint32", "uint256", "bytes", "address", "uint256"],
                [loanId, newDuration, renegotiateFee, encodedLenderSignature, directLoanFixedOffer.address, 31337]
            );

            const renegotiationMessage = ethers.utils.arrayify(ethers.utils.keccak256(renegotiationPayload));

            lenderSignature.signature = lender.signMessage(renegotiationMessage);

            await wXCR.connect(borrower).approve(directLoanFixedOffer.address, renegotiateFee);

            const loanContractOwner = await directLoanFixedOffer.owner();

            const adminFee = renegotiateFee.mul(offer.adminFeeInBasisPoints).div(10000);
            const lenderFee = renegotiateFee.sub(adminFee);

            await expect(directLoanFixedOffer.connect(borrower).renegotiateLoan(loanId, newDuration, renegotiateFee, lenderSignature.nonce, lenderSignature.expiry, lenderSignature.signature))
                .to.emit(directLoanFixedOffer, "LoanRenegotiated")
                .changeTokenBalances(wXCR, [borrower.address, lender.address, loanContractOwner], [renegotiateFee.mul(-1), lenderFee, adminFee]);

            const newLoan = await directLoanFixedOffer.loanIdToLoan(loanId);

            expect(newLoan.duration).to.be.equal(newDuration);
        });
    });
