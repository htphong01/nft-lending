...
export default function Faucet() {
  ...
  const handleMintERC20 = async () => {
    try {
      ...
      toast.success('Faucet 20 wXCR successfully. Please wait for state updated');
    } catch (error) {
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.content}>
        <div className={styles.title}>{pathname === '/wxcr-faucet' ? 'wXCR FAUCET' : 'NFT Faucet'}</div>
        <div className={styles.description}>
          Fast and reliable. {pathname === '/wxcr-faucet' ? '20 wXCR/day' : '5 NFTs/day'}
        </div>
        <div className={styles['input-control']}>
          ...
          <button disabled={address === ''} onClick={pathname === '/wxcr-faucet' ? handleMintERC20 : handleMintERC721}>
            {pathname === '/wxcr-faucet' ? 'Send wXCR' : 'Mint NFT'}
          </button>
        </div>
      </div>
    </div>
